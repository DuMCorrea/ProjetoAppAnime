...
export const TopAnimes = () => {
  ...
  return (
    <div className="top-animes">
      {topAnimesList && topAnimesList.data ? (
        <div>
          <h1>Nossos melhores animes:</h1>
          <label>
            Caso não tenha encontrado seu anime, volte a página inicial!
          </label>
          <button onClick={() => navigate("/animeslist")}>Voltar</button>
          {topAnimesList.data?.map((animes) => (
            <div>
              <h1>{animes.title}</h1>
              <img
                onClick={() => navigate(`/anime/top-animes/${animes.mal_id}`)}
